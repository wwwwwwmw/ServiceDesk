...
export const createTicket = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const ticketResult = await client.query(
      `INSERT INTO tickets (title, requester_id, creator_id, category_id, location_id, room, priority, status, dynamic_data) 
...
export const findUserByEmail = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = await query(
      `SELECT u.id, u.username, u.email, u.location_id, l.name as location_name, u.room 
       FROM users u
       LEFT JOIN locations l ON u.location_id = l.id
       WHERE u.email = $1`,
      [email]
