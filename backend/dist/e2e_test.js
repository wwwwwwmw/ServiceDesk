...
async function runTest() {
    ...
    try {
        ...
        const emailData = await loginEmailRes.json();
        ...
        const createBehalfData = await createBehalfRes.json();
        if (createBehalfRes.status === 201 && createBehalfData.ticket?.id) {
            ...
            const behalfDetail = await behalfDetailRes.json();
            if (behalfDetailRes.status === 200) {
                ...
                if (behalfDetail.requester_id === 'f3333333-3333-3333-3333-333333333333' &&
        ...
        if (detailRes.status === 200) {
            ...
            results.verifyDetail = (ticketDetail.status === 'closed' &&
                ...
                timeline.some((t) => t.type === 'comment' && t.content.includes('Vẫn chưa vào được mạng')) &&
                timeline.some((t) => t.type === 'log' && t.content.includes('Mở lại ticket')));
        }
        else {
